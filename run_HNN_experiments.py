from NN_models import HNN_for_vmap
...
for sim_name, sim_func in simulations.items():
    ...
    for run in range(NUM_RUNS):
        ...
        y_train_dfNN_predicted = vmap(HNN_model)(x_train.to(device)).detach()
        y_test_dfNN_predicted = vmap(HNN_model)(x_test.to(device)).detach()
        ...
        dfNN_train_div = torch.diagonal(vmap(jacfwd(HNN_model))(x_train.to(device)), dim1 = -2, dim2 = -1).detach().sum().item()
        dfNN_test_div = torch.diagonal(vmap(jacfwd(HNN_model))(x_test.to(device)), dim1 = -2, dim2 = -1).detach().sum().item()
