...
# RUN WITH python run_real_experiments_dfGPcm.py
#               _                 _   _      
#              | |               | | (_)     
...
if model_name in ["GP", "dfGP", "dfNGP", "dfGPcm"]:
...
for region_name in ["region_lower_byrd", "region_mid_byrd", "region_upper_byrd"]:
    ...
    # NOTE: GPs don't train on batches, use full data, even for dfGPcm
    ...
    for run in range(NUM_RUNS):
        ...
        ### Initialise dfGPcm hyperparameters ###
        ...
        for epoch in range(MAX_NUM_EPOCHS):
            ...
            if run == 0:
                ...
                mean_pred_train, _, lml_train = GP_predict(
                        ...
                        mean_func = mean_vector, # dfNN as mean function
        ...
        if run == 0:
            ...
            torch.save(dfGPcm_test_div_field, f"{MODEL_REAL_RESULTS_DIR}/{region_name}_{model_name}_test_prediction_divergence_field.pt")
        ...
        dfGPcm_train_NLL = compute_NLL_sparse(y_train, mean_pred_train, (covar_pred_train + torch.diag(train_noise_diag**2))).item()
        dfGPcm_train_full_NLL, dfGPcm_train_jitter = compute_NLL_full(y_train, mean_pred_train, (covar_pred_train + torch.diag(train_noise_diag**2)))

        dfGPcm_test_RMSE = compute_RMSE(y_test, mean_pred_test).item()
        dfGPcm_test_MAE = compute_MAE(y_test, mean_pred_test).item()
        dfGPcm_test_NLL = compute_NLL_sparse(y_test, mean_pred_test, (covar_pred_test + torch.diag(test_noise_diag**2))).item()
        dfGPcm_test_full_NLL, dfGPcm_test_jitter = compute_NLL_full(y_test, mean_pred_test, (covar_pred_test + torch.diag(test_noise_diag**2)))

        region_results.append([
            run + 1,
            dfGPcm_train_RMSE, dfGPcm_train_MAE, dfGPcm_train_NLL, dfGPcm_train_full_NLL.item(), dfGPcm_train_jitter.item(), dfGPcm_train_div,
            dfGPcm_test_RMSE, dfGPcm_test_MAE, dfGPcm_test_NLL, dfGPcm_test_full_NLL.item(), dfGPcm_test_jitter.item(), dfGPcm_test_div
        ])
