from archive.NN_models_HNN_dfNN_fullmatrix import HNN_for_vmap
...
from configs import PATIENCE, MAX_NUM_EPOCHS, NUM_RUNS, LEARNING_RATE, WEIGHT_DECAY, BATCH_SIZE, N_SIDE, HNN_RESULTS_DIR, DFNN_LEARNING_RATE
...
for sim_name, sim_func in simulations.items():
    ...
    for run in range(NUM_RUNS):
        ...
        HNN_model = HNN_for_vmap().to(device)
        HNN_model.train()
        ...
        for epoch in range(MAX_NUM_EPOCHS):
            ...
            y_test_pred = vmap(HNN_model)(x_test.to(device))
            ...
            if avg_train_loss < best_loss:
                ...
                best_model_state = HNN_model.state_dict()  # Save best model
            else:
                epochs_no_improve += 1
        ...
        HNN_model.load_state_dict(best_model_state)
        ...
        y_train_HNN_predicted = vmap(HNN_model)(x_train_grad)
        y_test_HNN_predicted = vmap(HNN_model)(x_test_grad)

        # Only save things for one run
        if run == 0:
            #(1) Save predictions from first run so we can visualise them later
            torch.save(y_test_HNN_predicted, f"{RESULTS_DIR}/{sim_name}_{model_name}_test_predictions.pt")
        ...
        u_indicator_train, v_indicator_train = torch.zeros_like(y_train_HNN_predicted), torch.zeros_like(y_train_HNN_predicted)
        u_indicator_train[:, 0] = 1.0 # output column u selected
        v_indicator_train[:, 1] = 1.0 # output column v selected

        HNN_train_div = (torch.autograd.grad(
            outputs = y_train_HNN_predicted,
            inputs = x_train_grad,
        ...
        u_indicator_test, v_indicator_test = torch.zeros_like(y_test_HNN_predicted), torch.zeros_like(y_test_HNN_predicted)
        u_indicator_test[:, 0] = 1.0 # output column u selected
        v_indicator_test[:, 1] = 1.0 # output column v selected

        HNN_test_div = (torch.autograd.grad(
            outputs = y_test_HNN_predicted,
            inputs = x_test_grad,
        ...
        HNN_train_RMSE = compute_RMSE(y_train, y_train_HNN_predicted.cpu()).item()
        HNN_train_MAE = compute_MAE(y_train, y_train_HNN_predicted.cpu()).item()

        HNN_test_RMSE = compute_RMSE(y_test, y_test_HNN_predicted.cpu()).item()
        HNN_test_MAE = compute_MAE(y_test, y_test_HNN_predicted.cpu()).item()

        # Store results in list
        simulation_results.append([
            run + 1, HNN_train_RMSE, HNN_train_MAE, HNN_train_div,
            HNN_test_RMSE, HNN_test_MAE, HNN_test_div
        ])
