...
for region_name in ["region_lower_byrd", "region_mid_byrd", "region_upper_byrd"]:
    ...
    for run in range(NUM_RUNS):
        ...
        # Increase for new domain size 
        # model.covar_module.base_kernel.lengthscale = model.covar_module.base_kernel.lengthscale * 5
        # model.likelihood.noise = model.likelihood.noise * 5
        # model.covar_module.outputscale = model.covar_module.outputscale
        ...
        model.base_kernel.lengthscale = torch.tensor([3.0, 6.0]).to(device)
        model.covar_module.outputscale = torch.tensor([1.0]).to(device).requires_grad_(False)
        model.likelihood.noise = torch.tensor([0.06]).to(device)
