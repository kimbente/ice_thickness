...
from configs import PATIENCE, MAX_NUM_EPOCHS, NUM_RUNS, GP_LEARNING_RATE, WEIGHT_DECAY, SIGMA_N_RANGE, SIGMA_F_RANGE, L_RANGE, B_DIAGONAL_RANGE, B_OFFDIAGONAL_RANGE, dfNGP_REAL_RESULTS_DIR
import torch
...
for region_name in ["regionc"]:
    ...
    RESULTS_DIR = dfNGP_REAL_RESULTS_DIR
    ...
    for run in range(NUM_RUNS):
        ...
        for epoch in range(MAX_NUM_EPOCHS):
            ...
            if run == 0:
                ...
                mean_pred_test, _, _ = GP_predict(
                        x_train,
                        y_train,
                        x_test, # have predictions for training data again
                        [sigma_n, sigma_f, l], # initial hyperparameters
                        dfNN_mean_model,
                        divergence_free_bool = True,
                        train_noise_input = train_noise_diagmatrix)
        ...
        if run == 0:
            ...
            torch.save(dfNGP_test_div_field, f"{RESULTS_DIR}/{region_name}_{model_name}_test_prediction_divergence_field.pt")
